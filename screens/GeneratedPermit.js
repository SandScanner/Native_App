import { View, Text, ScrollView, useWindowDimensions, TouchableHighlight, Image } from 'react-native'
import React, { useContext, useState, useRef, useEffect } from 'react'
import { AuthContext } from '../context/AuthContext';
import { th_logo } from '../assets';
import RNHTMLtoPDF from 'react-native-html-to-pdf';
import Pdf from 'react-native-pdf';
import QRCode from 'react-native-qrcode-svg'
import {BluetoothManager,BluetoothEscposPrinter, BluetoothTscPrinter } from 'react-native-bluetooth-escpos-printer';
import RNFetchBlob from 'rn-fetch-blob';
import { convert, convertB64, getBase64 } from 'react-native-pdf-to-image';
import Spinner from 'react-native-loading-spinner-overlay';
import axios from 'axios';
import { BASE_URL } from '../config';

const GeneratedPermit = ({navigation}) => {
    const [pdfPath, setPdfPath] = useState();
    const { vehicleData, userInfo, bluetoothInfo, initialPdf, setInitialPdf } = useContext(AuthContext);
    const gstin = '33AAAGE0327H1Z1';

    let dateOfBooking = new Date();
    ...
    // for vehicle number formatting
    var re = /(\d+)([a-zA-Z]+)/g; 
    var vehicleNumber = vehicleData?.vehicleRegNumber?.replace(re, ' $1 $2 ');
    // end of vehicle number formatting
    ...
    function formatDate(dateVal) {
        ...
        var sMonth = padValue(newDate.toLocaleString('en-US', { month: 'short' }));
    ...
    let formatted_approved_date = formatDate(currentTime);

    let deliveryAddress = `${vehicleData?.delivery},<br>${vehicleData?.city},<br>${vehicleData?.selectedDistrict} - ${vehicleData?.pincode}`

    let gst_price = (price == 7950 ? 189 : 126);
    ...
    const printPdf = async() => {
        setLoading(true);
        console.log("bluetoothub ", bluetoothInfo)
        if(!bluetoothInfo?.name){
            setLoading(false);
            navigation.navigate('connectPrinter')
            return; 
        }

        
        // const images = await convert("file:///"+pdfPath);
        // console.log('image path ', images.outputFiles)
        // setImgPath(images.outputFiles[0])
    
        // await pdfToBase64("file:///"+images.outputFiles)
        // .then(async(base64Data) => {
        // if (base64Data) {
        //     // Use the base64 data as needed (e.g., send it to a server or display it in your app)
        //     // console.log('PDF converted to Base64:', base64Data);
        //     await BluetoothEscposPrinter.printerInit();
            
        //     // await BluetoothEscposPrinter.printText(`PRINT\n${base64Data}\n`, {});
        //     await BluetoothEscposPrinter.printPic(base64Data, {left: 30});
        ...
        await pdfToBase64(pdfPath)
        .then(async(base64Data) => {
            if(base64Data){

                const images = await convertB64(base64Data, 3000);
                console.log('image path ', images.outputFiles)
                setImgPath(images.outputFiles[0])
    
                await pdfToBase64("file:///"+images.outputFiles)
                .then(async(base64Data1) => {
                if (base64Data1) {
                    // Use the base64 data as needed (e.g., send it to a server or display it in your app)
                    // console.log('PDF converted to Base64:', base64Data);
                    await BluetoothEscposPrinter.printerInit();
                    await BluetoothEscposPrinter.setWidth(670);
                    // await BluetoothEscposPrinter.printText(`PRINT\n${base64Data}\n`, {});
                    await BluetoothEscposPrinter.printPic(base64Data1, {});

                    // await BluetoothEscposPrinter.printText(`\x1B\x74\x44`, {});
                    
                    // await BluetoothEscposPrinter.printText(`கனிமம்`, {codepage: 68});
                    
                    axios.post(`${BASE_URL}/confirmAndCloseOrder`, {
                        permitId: vehicleData?.regNo,
    ...
    const getHTMLString = (img_src) => {
      ...
      <div class="clear">
          ...
          <div class="width100 center">
              <img src=${img_src} width="90" height="100" class="img-margin"/>
              <p class="font13"></p>
          </div>
          <p style="text-align: right; padding-right: 10px">அனுமதி எண். <strong>${vehicleData?.regNo}</strong></p>
      
          <div class="clear">
              <div class="left"><p>பதிவு எண். <strong>${vehicleData?.regNo + 6000000}</strong></p></div>
      ...
      <div class="rowWiseData width60">
          <div>1. <strong>${vehicleData?.surveyNumber}</strong></div>
          <div>2. <strong>${vehicleData?.VillageName}</strong></div>
          <div>3. அலகு - <strong>${vehicleData?.unit == 3 ? '8.49 m³' : '5.66 m³'}</strong></div>
          ...
          <div>5. (i) மணல் கொண்டு செல்வதற்கான அனுமதி எண் / நாள்
              <br><strong>${vehicleData?.regNo}/ ${formatted_dob}</strong></div>
          <div>(ii) வண்டி எண் <br><strong>${vehicleNumber}</strong></div>
          <div>(iii) மணல் கொண்டு செல்லுமிடம் <br><strong>${deliveryAddress}</strong></div>
          <div>6. அனுமதிதாரர் / உரிமை முகவரின் பெயரும் கையொப்பமும்<br><strong>${vehicleData?.customerName}</strong>
          </div>
  ...
  const generateQR = async() => {
    await qr_ref.toDataURL(callback);
}

async function createPDF(img_data) {

    let html_string = getHTMLString(img_data)
...
const callback = (dataurl) => {
    img_src = "data:image/png;base64,"+dataurl.replace(/(\r\n|\n|\r)/gm, "");
    createPDF(img_src);
}

const pdfToBase64 = async (pdfFilePath) => {
    try {
      const pdfData = await RNFetchBlob.fs.readFile(pdfFilePath, 'base64');
      return pdfData;
    } catch (error) {
...
//     });
    ...
    let random_str = makeid(32);
    console.log('randomess', random_str)    
    ...
    return (
        <ScrollView style={{ flex: 1 }}>
            ...
            <QRCode
            value={`https://www.google.com/search?q=${random_str}`}
            getRef={ref => {qr_ref = ref}}
            logo={th_logo}
            logoSize={40}
            color='black'
            ecl="Q"
            ...
            {!pdfPath && <TouchableHighlight onPress={generateQR} style={{backgroundColor: 'blue', width: "50%", height: 50, borderRadius: 50, alignItems: 'center', paddingTop: 12, alignSelf: 'center'}}>
                <Text style={{color: 'white'}}>Create Pdf</Text>
                
            </TouchableHighlight>}
            
            
            {pdfPath && <TouchableHighlight onPress={printPdf} style={{backgroundColor: 'blue', width: "50%", height: 50, borderRadius: 50, alignItems: 'center', paddingTop: 12, alignSelf: 'center'}}>
